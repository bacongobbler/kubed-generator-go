...
const (
	...
	globalUsage       = `Generates boilerplate code that is necessary to write a go app.
`
...
func newRootCmd(stdout io.Writer, stdin io.Reader, stderr io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "generator-go <name>",
		Short:        "generates go applications",
		Long:         globalUsage,
		SilenceUsage: true,
...
func (c *generateCmd) run() error {
	...
	p := &pack.Pack{
		Files: map[string]io.ReadCloser{
			...
			"main.go":    ioutil.NopCloser(bytes.NewBufferString(maingo)),
		},
	}
